...
def feature_extraction(input_data, stft_parameters=DEFAULT_STFT_PARAMETERS, data_configuration='10-10-20',
                       fs=SAMPLING_FQ, use_channel_inds=CHANNELS_IDX, add_feature=False):
    '''
...
def extract_label(input_array, input_length=2400):
    interval = (input_length * 150)
    interval = interval // 2400
    sample_num = (input_length * 30)
    sample_num = sample_num // 2400
...
def extract_data(input_array, input_length=2400):
    interval = (input_length * 150)
    interval = interval // 2400
    sample_num = (input_length * 30)
    sample_num = sample_num // 2400
...
svm_result = np.zeros([30,59])
xgb_result = np.zeros([30,59])
rf_result = np.zeros([30,59])

svm_duration = np.zeros([30,59])
xgb_duration = np.zeros([30,59])
rf_duration = np.zeros([30,59])

W_size = list(range(2,61,2))
W_shift = list(range(4,1281,22))
print("success in define all function !!!!!")

# extract the data and label

for w_size in W_size:
    print("Start processing %s loop of size" % (count_row+1))
    
    for w_shift in W_shift:
        print("Start processing %s loop of shift in %s loop of size" % (count_col+1, count_row+1))
        start_loop = default_timer()

        loop_STFT_PARAMETERS = {'restricted_frequencies': RESTRICTED_FQ,
                                'offset': 0,
                                'window_size': w_size,
        ...
        for name in Subject1:
            feature_subject1[name], label_subject1[name] = feature_extraction(
                d[name], stft_parameters=loop_STFT_PARAMETERS, data_configuration='10-10-20')
        ...
        for i in classifiers:
            if i[1] == 'SVM':
                ...
                svm_duration[count_row, count_col] = duration
                ...
                duration = default_timer() - start
                rf_duration[count_row, count_col] = duration
                ...
                duration = default_timer() - start
                xgb_duration[count_row, count_col] = duration
                
            print(i[1])

        # print("The accuracy of SVM classifier is %s and duration is %s" % (confusion_SVM_list_1_1[0].accuracy(), svm_duration))
        # print("The accuracy of XGB classifier is %s and duration is %s" % (confusion_XGB_list_1_1[0].accuracy(), xgb_duration))
        # print("The accuracy of RF classifier is %s and duration is %s" % (confusion_rf_list_1_1[0].accuracy(), rf_duration))
        ...
        print("The loop take %s seconds" % (default_timer() - start_loop))
...
for cou_col in range (59):
    name = str(cou_col)

    svm_dur[name] = svm_duration[:,cou_col]
    xgb_dur[name] = xgb_duration[:,cou_col]
    rf_dur[name] = rf_duration[:,cou_col]

df_svm_duration = pd.DataFrame(svm_dur)
df_xgb_duration = pd.DataFrame(xgb_dur)
df_rf_duration = pd.DataFrame(rf_dur)

cou_row = list(range(2,61,2))
cou_row_str = map(str,cou_row)

cou_col = list(range(4,1284,22))
cou_col_str = map(str,cou_col)

df_svm_duration.columns = list(cou_col_str)
df_xgb_duration.columns = list(cou_col_str)
df_rf_duration.columns = list(cou_col_str)

df_svm_duration.index = list(cou_row_str)
df_xgb_duration.index = list(cou_row_str)
df_rf_duration.index = list(cou_row_str)

excel_path_duration = 'get_result_duration.xlsx'

df_svm_duration.to_excel(excel_path_duration, sheet_name= 'Sheet1', index= True, header= True)
df_xgb_duration.to_excel(excel_path_duration, sheet_name= 'Sheet2', index= True, header= True)
df_rf_duration.to_excel(excel_path_duration, sheet_name= 'Sheet3', index= True, header= True)
...
df_svm.columns = list(cou_col_str)
df_xgb.columns = list(cou_col_str)
df_rf.columns = list(cou_col_str)
